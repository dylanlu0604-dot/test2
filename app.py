...
with st.sidebar:
    ...
    default_std_index = std_choices.index(2.0) if 2.0 in std_choices else 0
    std_value = st.selectbox("標準差門檻", options=std_choices, index=default_std_index)

    roll_choices = [6, 12, 24, 36, 60, 120]
    default_roll_index = roll_choices.index(120) if 120 in roll_choices else 0
    winrolling_value = st.selectbox("滾動期數", options=roll_choices, index=default_roll_index)
...
if resulttable1_list:
    ...
    pre = float(df.loc['mean', '-12m']) - 100 if 'mean' in df.index and '-12m' in df.columns else 0
    after = float(df.loc['mean', '12m']) - 100 if 'mean' in df.index and '12m' in df.columns else 0
    prewin = float(df.loc[WIN_RATE_LABEL, '-12m']) if WIN_RATE_LABEL in df.index and '-12m' in df.columns else 0
    afterwin = float(df.loc[WIN_RATE_LABEL, '12m']) if WIN_RATE_LABEL in df.index and '12m' in df.columns else 0
